...
public class PoliticianNetworkRepository {
    ...
    // User's provided API Key for NYS Legislation
    ...
    public PoliticianNetworkRepository(Politician_Dao dao) {
        ...
        // Retrofit for NYS Legislation API (requires key)
    ...
    /**
     * Executes the insertion of politicians on a background thread.
     * Note: We no longer use deleteAll() to preserve static data.
     */
    private void insertPoliticiansAsync(List<Politician> politicians, String source) {
        if (!politicians.isEmpty()) {
            new Thread(() -> {
                // FIX: Iterate and insert each politician individually
    ...
    // --- Parser for NYC Council (Socrata Array of Arrays) ---
    ...
    // --- Parser for NYS Legislators (Custom JSON Mapping) ---
